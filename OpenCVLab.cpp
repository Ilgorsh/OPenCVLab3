...
int stitch_(Mat &im1, Mat &im2) {
	Mat pano;
	vector<Mat> imgs = {im1,im2};
	Ptr<Stitcher> stitcher = Stitcher::create(mode);
	auto val = stitcher->stitch(imgs, pano);
	if (val != Stitcher::OK)
	{
...
vector<Point> skeleton(const vector<Point> input) {
...
int main()
{
	...
	//Sobel(gray);
	//Mat im1 = imread("1.jpg", IMREAD_COLOR);
	//Mat im2 = imread("2.jpg", IMREAD_COLOR);
	//stitch_(im1,im2);
